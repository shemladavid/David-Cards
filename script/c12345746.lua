...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local token = Duel.CreateToken(tp, 95788410)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 39674352)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 32626733)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 56649609)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 38999506)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 39111158)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 92176681)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 65957473)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 76232340)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 10949074)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 46986414)
    Duel.SendtoGrave(token, REASON_EFFECT)
    token = Duel.CreateToken(tp, 31447217)
    Duel.SendtoGrave(token, REASON_EFFECT)
end
...
function s.thtg(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then
        return Duel.IsExistingMatchingCard(s.thfilter, tp, LOCATION_GRAVE + LOCATION_DECK + LOCATION_REMOVED, 0, 1, nil)
    end
