...
function s.rcvtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local val=c:GetCounter(COUNTER_CAULDRON)*2000
	Duel.SetTargetPlayer(tp)
	Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,val)
end
function s.rcvop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local d=e:GetHandler():GetCounter(COUNTER_CAULDRON)*2000
...
function s.dmgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:GetCounter(COUNTER_CAULDRON)>0 end
	local dam=c:GetCounter(COUNTER_CAULDRON)*1200
	Duel.SetTargetPlayer(1-tp)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,dam)
end
function s.dmgop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local d=e:GetHandler():GetCounter(COUNTER_CAULDRON)*1200
	local p=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER)
