...
function s.abysskite(c)
	return c:IsMonster() and c:IsSetCard(SET_ABYSSKITE) and not c:IsCode(id)
end
function s.thfilter(c)
	return c:IsAbleToHand() and (s.abysskite(c) or c:IsCode(22977508))
end
...
function s.rescon(sg,e,tp,mg)
	return sg:FilterCount(Card.IsCode,nil,22977508)<=1
