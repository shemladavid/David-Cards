...
function s.initial_effect(c)
    ...
    for _, event in ipairs(events) do
        ...
        e2:SetCondition(s.maiden_counter_condition)
        e2:SetOperation(s.maiden_counter_operation)
        c:RegisterEffect(e2)
    end
    ...
    e3:SetCountLimit(1)
    e3:SetCondition(s.control_condition)
    e3:SetTarget(s.control_target)
    e3:SetOperation(s.control_operation)
    c:RegisterEffect(e3)
    ...
    e4:SetCategory(CATEGORY_TOGRAVE + CATEGORY_RECOVER + CATEGORY_SPECIAL_SUMMON)
    e4:SetType(EFFECT_TYPE_IGNITION)
    e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
    e4:SetRange(LOCATION_FZONE)
    e4:SetCondition(s.lp_gain_condition)
    e4:SetTarget(s.lp_gain_target)
    e4:SetOperation(s.lp_gain_operation)
    c:RegisterEffect(e4)
    ...
    e5:SetCondition(s.atk_boost_condition)
    e5:SetOperation(s.atk_boost_operation)
    c:RegisterEffect(e5)
    local e6 = e5:Clone()
