...
function s.initial_effect(c)
    ...
    c:RegisterEffect(e2)
    -- Add "frightfur", "edge imp", or "fluffal" card to hand
    ...
    c:RegisterEffect(e5)
    -- all "frightfur", "edge imp", and "fluffal" are treated as "frightfur" and "edge imp" and "fluffal"
    ...
    e6:SetValue(SET_FRIGHTFUR)
    c:RegisterEffect(e6)
    local e7 = e6:Clone()
    e7:SetValue(SET_EDGE_IMP)
    c:RegisterEffect(e7)
    local e8 = e6:Clone()
    e8:SetValue(SET_FLUFFAL)
    c:RegisterEffect(e8)
end
s.listed_series = {SET_FLUFFAL,SET_FRIGHTFUR,SET_EDGE_IMP}

function s.fusfilter(c)
	return c:IsType(TYPE_FUSION) and c:IsSetCard(SET_FRIGHTFUR)
end
...
function s.extrafil(e, tp, mg1)
	...
	-- (b) "frightfur" monsters in GY that can be shuffled into the Deck
...
function s.shufflefilter(c)
	return c:IsSetCard(SET_FRIGHTFUR) and c:IsMonster() and c:IsAbleToDeck()
end
...
function s.extratg(e, tp, eg, ep, ev, re, r, rp, chk)
	...
	Duel.SetOperationInfo(0, CATEGORY_TOGRAVE, nil, 0, tp,LOCATION_HAND + LOCATION_DECK + LOCATION_EXTRA + LOCATION_MZONE)
	Duel.SetOperationInfo(0, CATEGORY_TODECK, nil, 0, tp, LOCATION_GRAVE)
end


function s.thfilter(c)
    return (c:IsSetCard(SET_FRIGHTFUR) or c:IsSetCard(SET_EDGE_IMP) or c:IsSetCard(SET_FLUFFAL)) and c:IsAbleToHand()
end
...
function s.ngfilter(c)
    return c:IsFaceup() and (c:IsSetCard(SET_FRIGHTFUR) or c:IsSetCard(SET_EDGE_IMP) or c:IsSetCard(SET_FLUFFAL)) and c:IsMonster()
end
...
function s.spfilter(c, e, tp)
    return (c:IsSetCard(SET_FRIGHTFUR) or c:IsSetCard(SET_EDGE_IMP) or c:IsSetCard(SET_FLUFFAL)) and c:IsCanBeSpecialSummoned(e, 0, tp, false, false)
end
...
function s.target(e, c)
    return c:IsSetCard(SET_FRIGHTFUR) or c:IsSetCard(SET_EDGE_IMP) or c:IsSetCard(SET_FLUFFAL)
end